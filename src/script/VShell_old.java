...
public class VShell_old {
	private int history_count = 0;
	private Queue<String> cmd_historyQueue = new LinkedList();
	private Map<String, VCalculateInterface> mapAssignedMap = new HashMap<String, VCalculateInterface>();
	...
	private float Calc(VEasyFunctor vef, int deeps) {
		...
		for (Object str11 : strs) {
			...
			VEasyFunctor vef2 = (VEasyFunctor) this.mapAssignedMap.get(str11
					.toString().trim());
			float sFloat = 0.0F;

			sFloat = Calc(vef2, deeps + 1);

			vef.Given(str11.toString().trim(), Float.valueOf(sFloat));
		}

		return vef.Find().floatValue();
	}
	...
	public static void main(String[] args) throws IOException {
		VShell_old shell = new VShell_old();
